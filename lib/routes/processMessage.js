...
let appContext = require('../AmorphicContext').appContext;
 ...
 */
function processMessage(req, resp, sessions, controllers) {

    let session = req.session;
    let message = req.body;
    let path = url.parse(req.url, true).query.path;
    let sessionData = getSessionCache(path, req.session.id, false, sessions);
    let sendToLog = appContext.sendToLog;

    if (!message.sequence) {
        sendToLog(1, {sessionId: req.session.id, message: 'ignoring non-sequenced message'});
    ...
    establishServerSession(req, path, newPage, forceReset, message.rootId, sessions, controllers)
        ...
        // the next call into the server
            ...
            sendToLog(0, {sessionId: req.session.id, message: error.message + error.stack});
            resp.writeHead(500, {'Content-Type': 'text/plain'});
            resp.end(error.toString());
        }).done();
}
