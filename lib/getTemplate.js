...
function getTemplate(file, options, uses) {
    return function(ignoringClient, requiredTemplates, filesNeeded, ref, appPath, anotherPath, persistObjectTemplate, config, mixins, fileInitializers, filePaths, amorphicOptions, applicationSourceCandidate, currentContext, UsesV2ReturnPass1) {
        return _getTemplate(
            file,
            options,
            uses,
...
function _getTemplate(file, options, uses, ignoringClient, requiredTemplates, filesNeeded, ref, appPath, anotherPath, persistObjectTemplate, config, mixins, fileInitializers, filePaths, amorphicOptions, applicationSourceCandidate, currentContext, UsesV2ReturnPass1) {
    ...
    if (config.appConfig && config.appConfig.templateMode === 'auto') {
        ...
        var initializerReturnValues = fileInitializer(persistObjectTemplate,
            function usesV2Pass1(file, templateName, options) {
                templateName = templateName || getBasename(file);
                var moduleName = currentContext.moduleName;

                _getTemplate(file, options, true, ignoringClient, requiredTemplates, filesNeeded, ref, appPath, anotherPath, persistObjectTemplate, config, mixins, fileInitializers, filePaths, amorphicOptions, applicationSourceCandidate, currentContext, UsesV2ReturnPass1);
        ...
        var initializerGetTemplate = getTemplate();
        var templates = requiredTemplates[fileNoExt] || fileInitializer(persistObjectTemplate, getTemplateInject, usesV1);
    ...
    if (typeof(anotherPath) !== 'undefined') {
        if (amorphicOptions.sourceMode === 'debug') {
            applicationSourceCandidate[fileNoExt] = ["document.write(\"<script src='/" + clientPath + '/js/' + file + '?ver=' + config.appVersion + "'></script>\");\n\n"];
        }
        else {
            if (fileMixinsInitializer) {
                if (objectTemplateInitialize) {
                    applicationSourceCandidate[fileNoExt] = ['module.exports.' + fileNoExt + ' = ' + fileInitializer + '\n\n' +
                    'module.exports.objectTemplateInitialize = ' + objectTemplateInitialize + '\n\n' + 'module.exports.' + fileNoExt + '_mixins = ' + fileMixinsInitializer + '\n\n',
