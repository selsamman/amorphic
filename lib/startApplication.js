 ...
 */
function startApplication(appName, appDirectory, appList, configStore, sessionStore, amorphicOptions, applicationConfig, applicationSource, applicationSourceMap, applicationPersistorProps) {
    ...
    if (dbConfig.dbName && dbConfig.dbPath) {
        ...
        return dbClient.then(handleDBCase.bind(this, dbConfig, config, appName, path, cpath, schema, sessionStore, amorphicOptions, applicationConfig, nonObjTemplatelogLevel, applicationSource, applicationSourceMap, applicationPersistorProps)).catch(function errorrr(e) {
