...
function startApplication(appName, appDirectory, appList, configStore, sessionStore, amorphicOptions, applicationConfig,
    ...
    if (dbConfig.dbName && dbConfig.dbPath) {
        if (dbConfig.dbDriver == 'mongo') {
            ...
            // TODO: knex is already initialized because it is a synchronous function that is
            //       called when require('knex') occurs
            dbClient = Q(knex);
        }

        return dbClient.then(handleDBCase.bind(this, dbConfig, config, appName, path, cpath, schema, sessionStore,
            amorphicOptions, applicationConfig, nonObjTemplatelogLevel, applicationSource, applicationSourceMap,
            applicationPersistorProps)).catch(function errorrr(e) {
