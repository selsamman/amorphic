...
function getTemplate(file, options, uses) {
    return function getTemplateCurry(ignoringClient, requiredTemplates, filesNeeded, appPath, appName,
...
function _getTemplate(file, options, uses, ignoringClient, requiredTemplates, filesNeeded, appPath, appName,
                      persistObjectTemplate, config, mixins, fileInitializers, filePaths,
                      applicationSourceCandidate, currentContext, UsesV2ReturnPass1) {
    ...
    if (fileNoExt in requiredTemplates) {
        if (uses) {
            return;
        }
        else {
