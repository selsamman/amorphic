...
function loadTSTemplates(appName) {
    ...
    if (!controllerPath.match(/(.*?)([0-9A-Za-z_]*\.ts)$/)) {
        throw new Error('controllerPath (' + controllerPath +
            ') must specify a .ts file with templateMode of typescript');
    }
    ...
    AmorphicContext.applicationTSController[appName] = require(appConfig.appPath + '/' +
        controllerPath.replace(/.ts$/, '.js')).Controller;
