 ...
 */
function postRouter(sessions, controllers, nonObjTemplatelogLevel, sendToLog, req, resp, next) {

    if (req.url.match(/amorphic\/xhr\?path\=/) && url.parse(req.url, true).query.form && req.method === 'POST') {
        processPost(req, resp, sessions, controllers, nonObjTemplatelogLevel, sendToLog);
    }
    else {
        next();
    }
