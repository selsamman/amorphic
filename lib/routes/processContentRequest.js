 ...
 */
function processContentRequest(request, response, sessions, controllers) {

    let path = url.parse(request.url, true).query.path;

    establishServerSession(request, path, false, false, null, sessions, controllers).then(function zz(semotus) {
