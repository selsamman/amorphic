...
function getTemplates(persistObjectTemplate, appPath, templates, config, anotherPath, _sourceOnly, _detailedInfo,
  amorphicOptions, applicationSource, applicationSourceMap, applicationPersistorProps) {
    ...
    persistObjectTemplate.__statics__ = persistObjectTemplate.__statics__ || {}; //TODO: Are we always falling back to an empty object?
    ...
    if (config.appConfig && config.appConfig.templateMode === 'auto') {
        for (var fileInitializer in fileInitializers) {
            var oldCreate = persistObjectTemplate.create;

            persistObjectTemplate.create = function t(name, props) {
                name = name.name || name;
            ...
            recordStatics(fileInitializers[fileInitializer](persistObjectTemplate, usesV2Pass2));
    ...
    // Because of the two pass nature, requiredTemplates templates are not update for extends which are only done between passes
