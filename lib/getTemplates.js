 ...
 * @param {null} _sourceOnly - As of now, always passed null.
 * @param {null} detailedInfo - As of now, always passed null.
 *
...
function getTemplates(objectTemplate, appPath, templates, config, path, _sourceOnly, detailedInfo) {
    ...
    var ref = {};
    var mixins = [];
    var allRequireResults = {};
    var allFilePaths = {};
    var ignoringClient = false;
    var filesNeeded = {};
    var currentContext = {pass: 1};

    //TODO: Are we always falling back to an empty object?
    ...
    usesV2ReturnPass1.prototype.doExtend = function n(futureTemplates) {
        if (!objectTemplate.__dictionary__[this.baseName]) {
            ...
            if (!objectTemplate.__dictionary__[this.baseName]) {
                throw Error('Attempt to extend ' + this.baseName + ' which was never defined; extendedName=' +
                    this.extendedName);
            }
        }
    ...
    function getTemplate(file, options, uses) {
        ...
        if (options && options.app) {
            ...
            if (fs.existsSync(daemonPath)) {
                requireResults = require(daemonPath);
                requirePath = daemonPath;
            }
            else {
                requireResults = require(interactivePath);
                requirePath = interactivePath;
            }
        }
        else if (fs.existsSync(appPath + file)) {
            clientPath = path;
            requireResults = require(appPath + file);
            requirePath = appPath + file;
        }
        else {
            clientPath = 'common';
            requireResults = require(config.commonPath + file);
        ...
        var objectTemplateInitialize = requireResults['objectTemplateInitialize'];
        var initializer = (requireResults[prop]);
        var mixinsInitialzer = (requireResults[prop + '_mixins']);
        ...
        if (config.appConfig && config.appConfig.templateMode == 'auto') {
            (function o() {
                ...
                var initializerReturnValues = requireResults[prop](objectTemplate,
                ...
                allRequireResults[prop] = requireResults[prop];
            ...
            if (mixinsInitialzer && includeMixins) {
                mixins.push(mixinsInitialzer);
            }

            allRequireResults[prop] = initializer;

            if (mixinsInitialzer) {
                allRequireResults[prop + '_mixins'] = mixinsInitialzer;
            }
        }

        allFilePaths[prop] = requirePath;

        if (typeof(path) != 'undefined') {
            if (amorphicOptions.sourceMode == 'debug') {
                applicationSourceCandidate[prop] = ["document.write(\"<script src='/" + clientPath + '/js/' + file +
                '?ver=' + config.appVersion + "'></script>\");\n\n"];
            }
            else {
                if (mixinsInitialzer) {
                    if (objectTemplateInitialize) {
                        applicationSourceCandidate[prop] = ['module.exports.' + prop + ' = ' + requireResults[prop] +
                        '\n\n' + 'module.exports.objectTemplateInitialize = ' + objectTemplateInitialize + '\n\n' +
                        'module.exports.' + prop + '_mixins = ' + mixinsInitialzer + '\n\n',
    ...
    // Process V2 pass 2
    if (config.appConfig && config.appConfig.templateMode == 'auto') {
        for (var prop in allRequireResults) {
            var oldCreate = objectTemplate.create;
            ...
            recordStatics(allRequireResults[prop](objectTemplate, usesV2Pass2));
            objectTemplate.create = oldCreate;
        }
    }
    ...
    for (var propa in applicationSourceCandidate) {
        ...
        for (var template in requires[propa]) {
            if (requires[propa][template].__toClient__ ||
                typeof(requires[propa][template].__toClient__) == 'undefined') {
    ...
    if (config && config.appConfig && config.appConfig.modules) {
        for (var mixin in config.appConfig.modules) {
            if (!config.appConfig.modules[mixin].require) {
                ...
                logMessage(config.appConfig.modules[mixin].require + ' must export a ' + mixin +
                ...
                results[mixin + '_mixins'](objectTemplate, requires, config.appConfig.modules[mixin],
                    config.appConfig.nconf);

                if (typeof(path) != 'undefined') {
                    if (amorphicOptions.sourceMode == 'debug') {
                        applicationSource[path] += "document.write(\"<script src='/modules/" + requireName +
                            '/index.js?ver=' + config.appVersion + "'></script>\");\n\n";
                    }
                    else {
    ...
    if (ast && !applicationSource[path] && !config.appConfig.isDaemon) {
        ...
        applicationSourceMap[path] = sourceMap.toString();
    }
    ...
    if (detailedInfo) {
        ...
        detailedInfo.initializers = allRequireResults;
        detailedInfo.filePaths = allFilePaths;
    }
