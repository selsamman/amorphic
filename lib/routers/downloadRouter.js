 ...
 */
function downloadRouter(sessions, controllers, nonObjTemplatelogLevel, sendToLog, req, resp, next) {

    var file = url.parse(req.url, true).query.file;

    if (req.url.match(/amorphic\/xhr\?path\=/) && file && req.method === 'GET') {
        processContentRequest(req, resp, sessions, controllers, nonObjTemplatelogLevel, sendToLog);
    }
    else {
        next();
    }
