 ...
 */
function router(hostName, sessions, nonObjTemplatelogLevel, controllers, req, resp, next) {

    if (req.url.match(/amorphic\/xhr\?path\=/)) {
        if (req.body.type === 'logging') {
            processLoggingMessage(req, resp, hostName);
        }
        else {
            processMessage(req, resp, sessions, nonObjTemplatelogLevel, hostName, controllers);
        }
    }
    else {
        next();
