 ...
 */
function postRouter(applicationConfig, sessions, amorphicOptions, applicationSource, applicationSourceMap,
    ...
    if (req.url.match(/amorphic\/xhr\?path\=/) && url.parse(req.url, true).query.form && req.method == 'POST') {
        processPost(req, resp, applicationConfig, sessions, amorphicOptions, applicationSource, applicationSourceMap,
            applicationPersistorProps, hostName, controllers, nonObjTemplatelogLevel, sendToLog);
    }
    else {
        next();
