 ...
 */
function router(sendToLog, sessions, nonObjTemplatelogLevel, controllers, req, resp, next) {

    if (req.url.match(/amorphic\/xhr\?path\=/)) {
        if (req.body.type === 'logging') {
            processLoggingMessage(req, resp, sendToLog);
        }
        else {
            processMessage(req, resp, sessions, nonObjTemplatelogLevel, controllers, sendToLog);
        }
    }
    else {
        next();
