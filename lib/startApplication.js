...
function startApplication(appName, appDirectory, appList, configStore, sessionStore, amorphicOptions, applicationConfig,
    ...
    return setUpInjectObjectTemplate(appName, amorphicOptions, config, schema)
        .then(function buildAppConfigAndLoadTemplates(boundInjectObjectTemplateFunc) {
            applicationConfig[appName] = buildAppConfig(appName, amorphicOptions, config, controllerJsDir, commonJsDir,
                boundInjectObjectTemplateFunc, sessionStore);
            return loadTemplates(appName, applicationConfig[appName], amorphicOptions,
                applicationSource, applicationSourceMap, applicationPersistorProps)
        })
 ...
 * @param {String} appName - The app name.
 * @param {Object} appConfig - unknown
 * @param {Object} amorphicOptions - unknown
 * @param {Object} applicationSource - unknown
 * @param {Object} applicationSourceMap - unknown
 * @param {Object} applicationPersistorProps - unknown
 * @returns {[Object, Object]}
 */
function loadTemplates(appName, appConfig, amorphicOptions,
                       applicationSource, applicationSourceMap, applicationPersistorProps) {
    var dontMatter = null;
    ...
    return [
        baseTemplate,
        getTemplates(baseTemplate, appConfig.appPath, [prop + '.js'], appConfig, appName, dontMatter,
                    dontMatter, amorphicOptions, applicationSource, applicationSourceMap, applicationPersistorProps)
    ];
}
...
/**
 * Builds the applicationConfig for a given app.
 * @param {String} appName - The app name.
 * @param {String} amorphicOptions - unknown
 * @param {Object} config - The app specific config from the config store.
 ...
 */
function buildAppConfig(appName, amorphicOptions, config, path, commonPath, initObjectTemplateFunc, sessionStore) {
    return {
        appPath: path,
