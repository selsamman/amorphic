...
function buildStartUpParams(configStore) {
    ...
    for (let app in amorphicOptions.appStartList) {
        if (!amorphicOptions.appList[app]) {
            console.error(`FATAL your application: ${app} is not in the applications list in your root config.`);
        }
    }
