...
function getController(path, controllerPath, initObjectTemplate, connectSession, objectCacheExpiration, sessionStore,
                       newPage, reset, controllerId, req, applicationConfig, controllers, nonObjTemplatelogLevel,
                       amorphicOptions, applicationSource, applicationSourceMap, applicationPersistorProps, sessions,
                       sendToLog) {
    ...
    var templates = getTemplates(persistableSemotableTemplate, prefix, [prop + '.js'], config, path, null, null,
    ...
    var browser = ' - browser: ' + req.headers['user-agent'] + ' from: ' + (req.headers['x-forwarded-for'] ||
    ...
    if (!connectSession.semotus.controllers[path]) {
        ...
        if (newPage) {
            persistableSemotableTemplate.logger.info({component: 'amorphic', module: 'getController', activity: 'new',
                    controllerId: controller.__id__, requestedControllerId: controllerId || 'none'},
        ...
        persistableSemotableTemplate.withoutChangeTracking(function cc() {
            var sessionData = getSessionCache(path, sessionId, true, sessions);
            var unserialized = connectSession.semotus.controllers[path];

            controller = persistableSemotableTemplate.fromJSON(decompressSessionData(unserialized.controller),
                ControllerTemplate);

            if (unserialized.serializationTimeStamp != sessionData.serializationTimeStamp) {
                persistableSemotableTemplate.logger.error({component: 'amorphic', module: 'getController',
                        activity: 'restore', savedAs: sessionData.serializationTimeStamp,
            ...
            persistableSemotableTemplate.logger.info({component: 'amorphic', module: 'getController',
                    activity: 'restore'}, 'Restoreing saved controller ' + (newPage ? ' new page ' : '') + browser);
