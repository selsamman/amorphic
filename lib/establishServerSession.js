 ...
 *
 * @returns {Promise<Object>} Promise that resvoles to server session object.
 */
function establishServerSession(req, path, newPage, reset, newControllerId, applicationConfig, sessions,
    ...
    if (newPage === 'initial') {
        ...
        if (config.appConfig.createControllerFor && !session.semotus) {
            ...
            if (!referer.match(createControllerFor) && createControllerFor != 'yes') {
                ...
                return establishInitialServerSession(req, config, controllerPath, initObjectTemplate, path, time,
                    appVersion, sessionExpiration, applicationPersistorProps, amorphicOptions, applicationSource,
                    applicationSourceMap);
            }
        }
    }

    return establishContinuedServerSession(req, config, controllerPath, initObjectTemplate, path, time, appVersion,
        sessionExpiration, applicationPersistorProps, amorphicOptions, applicationSource, applicationSourceMap, session,
        sessionStore, newControllerId, hostName, objectCacheExpiration, newPage, applicationConfig, controllers,
        nonObjTemplatelogLevel, sessions, sendToLog, reset);
}
