...
function getController(path, controllerPath, initObjectTemplate, connectSession, objectCacheExpiration, sessionStore,
 ...
 -                    nonObjTemplatelogLevel);
    ...
    if (AmorphicContext.applicationTSController[path]) {                    // Typescript path
        templates = AmorphicContext.applicationTSController[path];
    }
    else {
        templates = getTemplates(persistableSemotableTemplate, prefix, [prop + '.js'], config, path);
    }
