...
function listen(appDirectory, sessionStore, preSessionInject, postSessionInject, sendToLogFunction) {
    ...
    for (let app in appStartList) {
        if (!appList[app]) {
            console.error('FATAL your main application is not in the applications list in your root config.');
            console.error('Amorphic is shutting down.');
            process.exit(1);
        }
    }
