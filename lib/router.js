 ...
 */
function router(hostName, applicationConfig, sendToLog, sessions, amorphicOptions, nonObjTemplatelogLevel, applicationSource, applicationSourceMap, applicationPersistorProps, controllers, req, resp, next) {
    if (req.url.match(/amorphic\/xhr\?path\=/)) {
        if (req.body.type == 'logging') {
            ...
            processMessage(req, resp, sessions, amorphicOptions, nonObjTemplatelogLevel, applicationConfig, applicationSource, applicationSourceMap, applicationPersistorProps, hostName, controllers, sendToLog);
        }
    }
    else {
        next();
